...
function sortearNumero() {
  ...
  resultInput.value = numeroSorteado.toString(8) // Mantém o valor octal
  inputNumber.value = ""
  labelResposta.innerHTML = "Resposta:"
  labelCalculo.innerHTML = ""

  // Mantém a definição da resposta correta conforme a opção selecionada
  const checkedOption = obterOpcaoSelecionada()
  if (checkedOption && checkedOption.value === "Decimal") {
    respostaCorreta = parseInt(numeroSorteado.toString(8), 8).toString()
  } else if (checkedOption && checkedOption.value === "Binario") {
    respostaCorreta = parseInt(numeroSorteado.toString(8), 8).toString(2)
  } else if (checkedOption && checkedOption.value === "Hexadecimal") {
    respostaCorreta = parseInt(numeroSorteado.toString(8), 8)
      .toString(16)
      .toUpperCase()
  }
}

function isValidOctal(octalStr) {
  return /^[0-7]+$/.test(octalStr)
}

function getOctalNumber() {
  const digitado = resultInput.value.trim()

  // Se vazio, usa o número sorteado
  if (!digitado) return numeroSorteado ? numeroSorteado.toString(8) : null

  // Validação do octal digitado
  if (!isValidOctal(digitado)) {
    alert("Por favor, digite um número octal válido (apenas dígitos 0-7)")
    return null
  }
