...
function sortearNumero() {
  ...
  // Mantenha a formatação original com 0x
  numeroHex = "0x" + numeroHex.padStart(Math.ceil(numeroHex.length), "0")

  numeroHexadecimal = numeroHex
  resultInput.value = numeroHex
  inputHex.value = ""
  labelResposta.textContent = "Resposta:"
  labelCalculo.textContent = ""

  // Mantenha a definição da resposta correta conforme a opção selecionada
  const checkedOption = obterOpcaoSelecionada()
  if (checkedOption === "Decimal") {
    respostaCorreta = parseInt(numeroHex.slice(2), 16).toString()
  } else if (checkedOption === "Binario") {
    respostaCorreta = parseInt(numeroHex.slice(2), 16).toString(2)
  } else if (checkedOption === "Octal") {
    respostaCorreta = parseInt(numeroHex.slice(2), 16).toString(8)
  }
}

  function isValidHex(hexStr) {
    // Aceita com ou sem 0x, apenas dígitos hexadecimais
    return /^(0x)?[0-9A-Fa-f]+$/i.test(hexStr)
  }

  function getHexNumber() {
    const digitado = resultInput.value.trim()

    // Se vazio, usa o número sorteado (compatibilidade com o original)
    if (!digitado) return numeroHexadecimal

    // Validação do hexadecimal digitado
    if (!isValidHex(digitado)) {
      alert("Por favor, digite um hexadecimal válido (ex: 1F ou 0x1F)")
      return null
    }

    // Garante que tem o prefixo 0x para consistência
    return digitado.startsWith("0x") ? digitado : "0x" + digitado
  }
...
function confirmarNumero() {
  const hexParaCalcular = getHexNumber() // Agora lê digitado ou sorteado

  if (!hexParaCalcular) {
    alert("Primeiro, digite um número hexadecimal válido ou clique em SORTEAR!")
    return
  }
  ...
  // Toda a lógica original abaixo é mantida!
  let respostaUsuario = inputHex.value.trim().toUpperCase()
  let respostaEsperada = ""
  let calculo = ""

  if (checkedOption === "Decimal") {
    const resultado = hexToDecimal(hexParaCalcular)
    respostaEsperada = resultado.resultado.toString()
    calculo = resultado.expressao
  } else if (checkedOption === "Binario") {
    const resultado = hexToBinary(hexParaCalcular)
    respostaEsperada = resultado.resultado
    calculo = `${resultado.primeiraForma}<br><br>${resultado.segundaForma}`
  } else if (checkedOption === "Octal") {
    const resultado = hexToOctal(hexParaCalcular)
    respostaEsperada = resultado.resultado
    calculo = resultado.calculoCompleto
  }

  // Verificação original (sem mudanças)
